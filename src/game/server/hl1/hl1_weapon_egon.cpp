...
ConVar sk_max_uranium("sk_max_uranium", "999", FCVAR_REPLICATED);

ConVar sk_plr_dmg_egon_wide(
	"sk_plr_dmg_egon_wide",
	"12",
	FCVAR_REPLICATED | FCVAR_CHEAT,
	"Damage per tick of Egon wide beam"
);
...
void CWeaponEgon::PrimaryAttack(void)
{
	...
	case FIRE_STARTUP:
	{
		...
		if (gpGlobals->curtime >= (m_flStartFireTime + 2.0))
		{
			...
			int seq = LookupSequence("hold_idle");
			...
			{
				Msg("Couldn't find the sequence hold_idle for gluon gun!\n");
			}
